...
const Reviewitems = ({items, onSmash}) => {
  const isMobile=useMediaQuery({maxWidth: 767})
  const isTablet=useMediaQuery({minWidth: 768, maxWidth:991})
  const isDesktop=useMediaQuery({minWidth:992})
  const Price=items.reduce((sum, item)=>sum + Number(item.sale), 0)
    ...
    <div className='review-overlay'>
      <div className='review-content'>
        ...
        <ul className='Reviewitem-container'>
          {items.map(item=>
            <>
              <li className='confirmItems-container'>
                <div className='reviews-cart'>
                  <div className="divImg">
                    {isMobile && <img className='img' src={item.image.mobile} />}
                    {isTablet && <img className='img' src={item.image.tablet} />}
                    {isDesktop && <img className='img' src={item.image.desktop} />}
                  </div>
                  <div className="confirmitems">
                    <div className="confirmItem-name">
                      {item.name}
                    </div>
          ...
          <div className='div-totalItem'>
            <div className="review-total">Order Total</div>
            <div className='totel-item'>${Price.toFixed(2)}</div>
          </div>
        </ul>
          <div className='start-new-orderBtn' onClick={()=>onSmash()}>
            Start New Order
          </div>
      </div>
    </div>
